...
cur = conn.cursor()
cur.execute('''CREATE TABLE IF NOT EXISTS data(tour TEXT, duration TEXT, reason TEXT, number TEXT)''')
conn.commit()
...
def add():
    ...
    line["number"] = 100
    print(getNumberFields())
    addedTour = line["tour"]
    addedDuration = line["duration"]
    addedReason = line["reason"]
    addedNumber = line["number"]
    # cur.execute('''INSERT INTO data (tour,duration,reason,number) VALUES(?,?,?,?)''',(addedTour,addedDuration,addedReason,addedNumber))
    tree.insert("",0,text=addedTour,values=(addedDuration,addedReason,addedNumber))

def validEntry(entry):
    if entry == 0 or entry is None or entry == "":
        return False
    return True

def checkAllEntries():
    fields = [beh1_field.get(),beh2_field.get(),beh3_field.get(),beh4_field.get()]
    if (fields[0] and fields[1] and fields[2] and fields[3]) == 0 or (fields[0] and fields[1] and fields[2] and fields[3]) == "" or (fields[0] and fields[1] and fields[2] and fields[3]) == 0:
        return False
    return True
...
def getNumberFields():
    ...
    found = []
    for i in range(4):
        if validEntry(fields[i]):
            found.append((i,fields[i]))
        else:
            continue
    return found
